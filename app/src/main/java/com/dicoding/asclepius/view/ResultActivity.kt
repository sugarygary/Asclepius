            ...
            when (intent.getStringExtra(EXTRA_LABEL)) {
                "Cancer" -> {
                    indicator.progressDrawable = AppCompatResources.getDrawable(
                        this@ResultActivity, R.drawable.progress_circle_cancer
                    )
                    tvGuide.apply {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                            justificationMode = LineBreaker.JUSTIFICATION_MODE_INTER_WORD
                        }
                        text = getString(R.string.cancer_guide)
                    }
                }

                else -> {
                    indicator.progressDrawable = AppCompatResources.getDrawable(
                        this@ResultActivity, R.drawable.progress_circle_non_cancer
                    )
                    tvGuide.apply {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                            justificationMode = LineBreaker.JUSTIFICATION_MODE_INTER_WORD
                        }
                        text = getString(R.string.non_cancer_guide)
                    }
                }
            }
